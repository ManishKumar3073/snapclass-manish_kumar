...
def teacher_tab_take_attendance():
    ...
    col1, col2 =st.columns([3,1],vertical_alignment='bottom') # size in terms of ratio
        ...
        if st.button('Run face Analysis',width='stretch',type='tertiary',icon=':material/analytics:',disabled=not has_photos):
            with st.spinner('Deep scanning classroom photos...'):
                ...
                for idx, img in enumerate(st.session_state.attendance_images):
                    ...
                    detedcted, _, _ = predict_attendance(img_np)
                    if detedcted:
                        for sid in detedcted.keys():
                            student_id = int(sid)
...
def teacher_tab_manage_subjects():
    ...
    if subjects:
        ...
        st.info(st.warning("No Subject Found. Create one Above"))
